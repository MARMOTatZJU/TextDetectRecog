...
class SSD300(nn.Module):
    # Adapted for INPUT_SIZE=(384,384)
    steps = (8, 16, 32, 64, 100, 300) # steps = (8, 16, 32, 64, 100, 300)
    box_sizes = (38, 77, 144, 211, 278, 345, 412) # box_sizes = (30, 60, 111, 162, 213, 264, 315)
    aspect_ratios = ((2,3,5),)*6 # aspect_ratios = ((2,), (2,3), (2,3), (2,3), (2,), (2,))
    fm_sizes = (48, 24, 12, 6, 4, 2)# fm_sizes = (38, 19, 10, 5, 3, 1)
    num_anchors_per_region = sum([2+2*len(ar) for ar in aspect_ratios])

    def __init__(self, num_anchor_pred=NUM_ANCHOR_PRED, num_classes=NUM_CLASSES, ):
        super(SSD300, self).__init__()
        self.num_anchor_pred = num_anchor_pred
        self.num_classes = num_classes
        self.num_anchors = (self.num_anchors_per_region,)*6 # (4, 6, 6, 6, 4, 4)
        ...
        for i in range(len(self.in_channels)):
        	self.loc_layers += [nn.Conv2d(self.in_channels[i], self.num_anchors[i]*self.num_anchor_pred, kernel_size=3, padding=1)]
    ...
    def forward(self, x):
        ...
        for i, x in enumerate(xs):
            ...
            loc_preds.append(loc_pred.view(loc_pred.size(0),-1,self.num_anchor_pred))
...
def test():
    print('test for SSD300, class_num=%d'%NUM_CLASSES)
    net = SSD300()
    loc_preds, cls_preds = net(Variable(torch.randn(*((1,3)+INPUT_SIZE))))
    print('(#BATCH,#ANCHORS,#COORD),(#BATCH,#ANCHORS,#CLASSES)')
